/**
 * Poll PinGlass v2 mockup tasks and download results
 */
...
const tasksPath = path.join(__dirname, '..', 'NewDesign', 'pinglass-v2-tasks.json');
...
async function pollAll(maxAttempts = 20) {
  let attempts = 0;
  let allComplete = false;
  const completed = new Set();

  // Create output directory
  const outDir = path.join(__dirname, '..', 'NewDesign', 'web', 'mockups-v2');
        ...
        if (result.code === 200 && result.data) {
          ...
          if (status === 'success' || status === 'completed') {
            ...
            if (imageUrl) {
              ...
              if (data.output) console.log(`   Output keys: ${Object.keys(data.output).join(', ')}`);
            }
            ...
            completed.add(task.taskId); // Don't retry failed
          } else {
  ...
  if (completed.size === tasks.length) {
    console.log(`\n✅ All mockups generated and saved to: ${outDir}`);
  } else {
    console.log(`\n💡 Run again for more attempts: node scripts/poll-pinglass-mockups.js`);
  }
...
const maxAttempts = parseInt(process.argv[2]) || 20;
pollAll(maxAttempts);
